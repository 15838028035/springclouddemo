...
//				cont.append('<div class="keytitle">资源名:' + keytitle + '</div>');
	...
	//显示视频方法
		...
		$("#menu-container").on('click', "#menuSave", function() {
            ...
            if (nowWeixinMenuId == 0){
                $.ajax({
                    url: ctxAppWeixin+'/wechat/menu/saveinmyservice?menuName='+menuName+'&remarks='+remarks,
                    type: "post",
                    ...
                    success: function(result) {
                        ...
                        if(result.respCode == 0) {
                            layer.msg("菜单保存成功，请点击同步按钮到微信官网！");
                            nowWeixinMenuId = result.respData;
                        } else {
                            layer.msg("菜单保存失败");
                        }
                    }
                ...
                $.ajax({
                    url: ctxAppWeixin+'/wechat/menu/saveinmyserviceupdate?menuid='+nowWeixinMenuId+'&menuName='+menuName+'&remarks='+remarks,
                    type: "post",
                    data: {"menus":JSON.stringify(menus)},
                    dataType: "json",
                    //contentType: "application/json; charset=utf-8",
